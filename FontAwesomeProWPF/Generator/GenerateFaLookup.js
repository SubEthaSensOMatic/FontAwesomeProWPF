...
fs.readFile(sourceFile, { encoding: 'utf-8' }, (err, json) => {
    ...
    if (!err) {
        ...
        cs += 'public static readonly Dictionary<string, char> Duotone = new Dictionary<string, char>() \r\n';
        cs += '{\r\n';

        for (var key in awesome) {
            if ('duotone' in awesome[key].svg)
                cs += '{"' + key + '",  \'\\u' + awesome[key].unicode + '\'},\r\n';
        }

        cs += '};\r\n';

        cs += 'public static readonly Dictionary<string, char> Light = new Dictionary<string, char>() \r\n';
        cs += '{\r\n';

        for (var key in awesome) {
            if ('light' in awesome[key].svg)
                cs += '{"' + key + '",  \'\\u' + awesome[key].unicode + '\'},\r\n';
        }
